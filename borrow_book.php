...
<body background="picture/library.png">
	...
	<div id="library">
		<div class="form"><div class="item">
			...
			$sql = "SELECT * FROM book where bno = '$bno' and stock > 0";
			...
			if ($result->num_rows > 0) {
				...
				values('$cno','$bno','$date','$return_date','$admin_id')";
				$conn->query($sql);
				echo "<p align='center'>"."借书成功"."</p>";
			} else {
				echo "<p align='center'>"."借书失败"."<br>";
				$sql = "SELECT min(return_date) as min FROM borrow where bno = '$bno' and return_date> '$date'";
				$result = $conn->query($sql);
				if ($result->num_rows > 0)
				{
					$row = $result->fetch_assoc();
					if($row["min"]!=NULL)echo "最近的归还时间是" . $row["min"]. "<br>"."</p>";
					else echo "该书不存在<br>"."</p>";
				}
			}
			mysqli_close($conn);
